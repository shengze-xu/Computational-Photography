...
void Image::generatePointCloud(string path,struct Camera camera)
{
	...
	//Mat frequency = setFrequencyUseSobel(depth);
	//srand(0);
	...
	//destroyAllWindows();
	//FILE *file;
	//fopen_s(&file, "D:/sample.txt", "w");
	//for (int i = 0; i < pointcloud.size(); i++)
	//{
	//	fprintf(file,"%lf %lf %lf %d %d %d\n", pointcloud[i].x2, pointcloud[i].y2, pointcloud[i].z2, pointcloud[i].r, pointcloud[i].g, pointcloud[i].b);
	//}
	//fclose(file);
}
...
unsigned int MIN_FREQUENCY = 0x1FF;
unsigned int MAX_FREQUENCY = 0x1FFF;
...
const uint LAP_THRESHOLD = 0xFFFF;
Mat Image::setFrequencyUseLaplacian(Mat depth)
{
	...
	Mat lap;
	Laplacian(depth2, lap, CV_32F, 7, 1);
	int border = 20;
	for (int i = 0; i < depth2.rows; i++)
	{
		for (int j = 0; j < depth2.cols; j++)
...
ARG arg[8];
void* routine_calconfidence(void* arg)
{
	ARG a = *(ARG*)arg;
	int size = a.p_ts->indexs.size();
	int from = size / 8 * a.n;
	int to = from + size / 8;
	int range = a.range;
	printf("%d:%d,%d\n",a.n, from, to);
	for (int i = from; i < to; i++)
	{
		...
		if (begin < 0) begin = 0;
		if (end >= a.p_ts->indexs.size()) end = a.p_ts->indexs.size() - 1;
		int idx = a.p_ts->indexs[i];
		for (int j = begin; j <= end; j++)
		{
			if (j == i) continue;
			int j_idx = a.p_ts->indexs[j];
			printf("calculating confidence of points %d referencing to %d\n", i, j);
			a.p_ts->images[idx].calculateConfidence(a.p_ts->images[j_idx], a.p_ts->path, a.p_ts->camera);
		}
	}
	return NULL;
}
...
{
	printf("start calculate confidence\n");
	...
	for (int ix = 0; ix < 8; ix++)
	{
		...
		arg[ix].p_ts = this;
		arg[ix].range = range;
		pthread_create(&thread[ix], NULL, routine_calconfidence, &(arg[ix]));
	}

	int *thread_ret = NULL;
	for (int ix = 0; ix < 8; ix++)
	{
		pthread_join(thread[ix], (void**)&thread_ret);
	}
	/*for (int i = 0; i < indexs.size(); i++)
	{
		int begin, end;
...
void TestSet::writeHashCloud(std::string filename)
{
	...
	fclose(file);
	printf("finish write hash cloud\n");
}
